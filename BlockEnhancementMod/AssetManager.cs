...
namespace BlockEnhancementMod
{
    public class AssetManager : SingleInstance<AssetManager>
    {
        ...
        private Dictionary<string, List<string>> readAudioClips()
        {
            ...
            var dirs = Modding.ModIO.GetDirectories(AudioFileRootPath, Data).ToList();
        ...
        {
            //obj文件路径
            //string filePath = @"Resources\Wheel.obj";
            string filePath = Application.dataPath + @"/Mods/BlockEnhancementMod/" + path;
            //获取obj文件文本
            //string fileText = ReadFile(filePath);
            string fileText = Modding.ModIO.ReadAllText(filePath, false);

            Mesh mesh = new Mesh();

            //解析obj字符串
            ObjMesh objMesh = new ObjMesh().LoadFromObj(fileText);
            mesh.vertices = objMesh.VertexArray;
            mesh.normals = objMesh.NormalArray;
            mesh.triangles = objMesh.TriangleArray;
            mesh.uv = objMesh.UVArray;
        ...
        {
            if (!Modding.ModIO.ExistsDirectory(AudioFileRootPath, Data))
            {
                Modding.ModIO.CreateDirectory(AudioFileRootPath, Data);
            }
